...
save_config() {
    local port=$1
    if [[ ! -f ~/domains/qin.ydns.eu/vless/config.json ]]; then
        uuid=$(generate_uuid)
        cat <<EOL > ~/domains/qin.ydns.eu/vless/config.json
{
    "uuid": "$uuid",
    "port": $port
}
EOL
        ...
        jq --arg port "$port" '.port = ($port | tonumber)' ~/domains/qin.ydns.eu/vless/config.json > ~/domains/qin.ydns.eu/vless/config_tmp.json && mv ~/domains/qin.ydns.eu/vless/config_tmp.json ~/domains/qin.ydns.eu/vless/config.json
...
deploy_vless() {
    ...
    npm install
    # 启动vless项目
    ~/.npm-global/bin/pm2 start ~/domains/qin.ydns.eu/vless/app.js --name vless
    # 保存pm2进程状态
    ...
    echo -e "域名: ${GREEN}qin.ydns.eu${NC}"
    echo -e "vless进程维护定时任务脚本: ${GREEN}cd ~/domains/qin.ydns.eu/vless && ./check_vless.sh${NC}"
    echo -e "VLESS节点信息: ${GREEN}vless://${uuid}@qin.ydns.eu:${port}?flow=&security=none&encryption=none&type=ws&host=qin.ydns.eu&path=/&sni=&fp=&pbk=&sid=#$USER.serv00.vless${NC}"
}
...
main() {
    ...
    if [ "$port_provided" = true ]; then
        ...
        if [[ -f config.json ]]; then
            ...
            echo -e "UUID: ${uuid}"
            echo -e "Port: ${port}"
            echo -e "域名: $qin.ydns.eu"
            echo -e "VLESS节点信息: vless://${uuid}@$qin.ydns.eu:${port}?flow=&security=none&encryption=none&type=ws&host=qin.ydns.eu&path=/&sni=&fp=&pbk=&sid=#qin.ydns.vless"
