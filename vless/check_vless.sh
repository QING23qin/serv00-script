...
save_config() {
    local port=$1
    if [[ ! -f ~/domains/$USER.ydns.eu/vless/config.json ]]; then
        uuid=$(generate_uuid)
        cat <<EOL > ~/domains/$USER.ydns.eu/vless/config.json
{
    "uuid": "$uuid",
    "port": $port
}
...
deploy_vless() {
    ...
    npm install
    # 启动vless项目
    ~/.npm-global/bin/pm2 start ~/domains/$USER.ydns.eu/vless/app.js --name vless
    # 保存pm2进程状态
    ...
    echo -e "VLESS节点信息: ${GREEN}vless://${uuid}@$USER.ydns.eu:${port}?flow=&security=none&encryption=none&type=ws&host=$USER.ydns.eu&path=/&sni=&fp=&pbk=&sid=#$USER.ydns.vless${NC}"
}
...
main() {
    ...
    if [ "$port_provided" = true ]; then
        ...
        if [[ -f config.json ]]; then
            ...
            echo -e "VLESS节点信息: vless://${uuid}@$USER.ydns.eu:${port}?flow=&security=none&encryption=none&type=ws&host=$USER.ydns.eu&path=/&sni=&fp=&pbk=&sid=#$USER.ydns.vless"
